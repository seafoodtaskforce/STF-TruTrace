...
struct ProfileDocumentDetail: View {
    ...
    func deletePageItems(at offsets: IndexSet){
        if let index = sessionData.profileDocumentList.firstIndex(of: documentCard) {
            for pageIndex in offsets {
                sessionData.profileDocumentList[index].pages[pageIndex].deleted = true
            }
            //sessionData.profileDocumentList[index].pages.remove(atOffsets: offsets)
        }
    }
    ...
    func deleteRecipientItems(at offsets: IndexSet){
        if let index = sessionData.profileDocumentList.firstIndex(of: documentCard) {
            for recipientIndex in offsets {
                sessionData.profileDocumentList[index].toRecipients[recipientIndex].deleted = true
            }
            // sessionData.profileDocumentList[index].toRecipients.remove(atOffsets: offsets)
        }
    }
    ...
    func deleteLinkedDocItems(at offsets: IndexSet){
        if let index = sessionData.profileDocumentList.firstIndex(of: documentCard) {
            for linkedIndex in offsets {
                sessionData.profileDocumentList[index].linkedDocuments[linkedIndex].deleted = true
            }
            // sessionData.profileDocumentList[index].linkedDocuments.remove(atOffsets: offsets)
        }
    }
    ...
    func deleteBackingDocItems(at offsets: IndexSet){
        if let index = sessionData.profileDocumentList.firstIndex(of: documentCard) {
            for attachedIndex in offsets {
                sessionData.profileDocumentList[index].attachedDocuments[attachedIndex].deleted = true
            }
            //sessionData.profileDocumentList[index].attachedDocuments.remove(atOffsets: offsets)
        }
    }
    ...
    func deleteTagItems(at offsets: IndexSet){
        if let index = sessionData.profileDocumentList.firstIndex(of: documentCard) {
             for tagIndex in offsets {
                sessionData.profileDocumentList[index].tags[tagIndex].deleted = true
            }
            // sessionData.profileDocumentList[index].tags.remove(atOffsets: offsets)
        }
    }
    ...
    func getDocumentPages() -> [DocumentPage]{
        if let index = sessionData.profileDocumentList.firstIndex(of: documentCard) {
            let activePages = sessionData.profileDocumentList[index].pages.filter {
                switch $0.deleted {
            ...
            return activePages
            // return sessionData.profileDocumentList[index].pages
        }
        return [DocumentPage]()
    }
    ...
    func getDocumentTags() -> [DocumentTag]{
        if let index = sessionData.profileDocumentList.firstIndex(of: documentCard) {
            ...
            return activeTags
            //return sessionData.profileDocumentList[index].tags
        }
        return [DocumentTag]()
    }
    ...
    func getDocumentBackingDocs() -> [DocumentDTO]{
        if let index = sessionData.profileDocumentList.firstIndex(of: documentCard) {
            ...
            //return sessionData.profileDocumentList[index].attachedDocuments
        }
        return [DocumentDTO]()
    }
    ...
    func getDocumentLinkedDocs() -> [DocumentDTO]{
        if let index = sessionData.profileDocumentList.firstIndex(of: documentCard) {
            ...
            //return sessionData.profileDocumentList[index].linkedDocuments
        }
        return [DocumentDTO]()
    }
    ...
    func getDocumentRecipients() -> [Recipient]{
        if let index = sessionData.profileDocumentList.firstIndex(of: documentCard) {
            ...
            //return sessionData.profileDocumentList[index].toRecipients
        }
        return [Recipient]()
    }
    ...
    func uploadUpdatedDocument(index : Int, pages : [DocumentPage])  {
        ...
        if let encoded = try? jsonEncoder.encode(sessionData.profileDocumentList[index]) {
            jsonBody = encoded
        }
        ...
        URLSession.shared.dataTask(with: request) {
            ...
            DispatchQueue.main.async {
                    ...
                    //
                    // Put the pages back
                    self.sessionData.profileDocumentList[index].pages = pages
                    ...
                    for docPage in pages {
                        if(docPage.id <= 0 && docPage.deleted == false){
                            ...
                            self.loadPageImage(page: docPage, sessionId : self.sessionData.profileDocumentList[index].syncID)
                        }
                        if(docPage.deleted == true && docPage.id > 0){
                            print("Pages <doc> <delete> \(docPage)")
                            self.deletePageImage(page: docPage, sessionId : self.sessionData.profileDocumentList[index].syncID)
                        }
                    }
            }
        }.resume()
