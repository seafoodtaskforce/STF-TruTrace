...
struct UserProfileView: View {
    ...
    var body: some View {
        NavigationView {
            VStack {
                ...
                VStack{
                    ImageViewWidget(imageUrl: ImageUtils.fetchProfileURL(username : self.sessionData.userCredentials.username))
                    Button(action: {
    ...
    func saveImageToServer(image : UIImage){
        //
        // Shrin the image
        var resizedImage = image.resized(toWidth: 320.0)
        
        let boundary = UUID().uuidString
        let filename = "profile.jpg"
        let urlString = RESTServer.REMOTE_SERVER_URL + RESTServer.REST_USER_POST_PROFILE_IMAGE
