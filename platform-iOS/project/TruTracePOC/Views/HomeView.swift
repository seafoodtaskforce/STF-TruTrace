...
struct HomeView: View {
    ...
    let timer = Timer.publish(every: LocalNotificationManager.NOTIFICATION_INTERVAL_SEC, on: .main, in: .common).autoconnect()
    ...
    func checkForNotificationData() {
        guard let url = URL(string:
            (sessionData.serverURL + RESTServer.REST_GET_NOTIFICATIONS_FOR_USER_ALL))
            ...
            let decodedResponse = try! JSONDecoder().decode([NotificationDTO].self, from: data)
                ...
                DispatchQueue.main.async {
                    ...
                    for notification in decodedResponse {
                        var title : String
                        var body : String
                        var docSessionId : String = ""
                        
                        //
                        // set data
                        title = notification.auditData.actor.name + " " + notification.auditData.action + " " + notification.auditData.itemId
                        body = "Some Body"
                        docSessionId = notification.auditData.itemId
                        self.setNotification(title:title, body : body, docId : docSessionId)
    ...
    //
    //
    func setNotification(title : String, body : String, docId: String) -> Void {
        let manager = LocalNotificationManager()
        manager.requestPermission()
        manager.addNotification(title: title, body : body, docId: docId)
        manager.scheduleNotifications()
    }
    
    //
    //
    // Offline Records Check
    func checkForOffflineRecords(){
        print("<offline storage> \(UserDefaults.standard.dictionaryRepresentation().keys)")
    }
