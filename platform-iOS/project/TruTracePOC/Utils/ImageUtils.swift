...
class ImageUtils {
    ...
    static func fetchProfileURL(username : String, sessionData: SessionData) -> String {
        let imageFetchURL = (sessionData.serverURL + RESTServer.REST_USER_GET_PROFILE_IMAGE)
        let imageFetchURLReplaced = imageFetchURL.replacingOccurrences(of: "{username}", with: username)
        return imageFetchURLReplaced
    }
    
    static func fetchDocumentPageURL(pageId : Int, sessionData: SessionData) -> String {
        let imageFetchURL = (sessionData.serverURL + RESTServer.REST_USER_GET_DOCUMENT_PAGE)
        let imageFetchURLReplaced = imageFetchURL.replacingOccurrences(of: "{doc_id}", with: String(pageId))
        return imageFetchURLReplaced
    }
    
    static func fetchDocumentPageThumbailURL(pageId : Int, sessionData: SessionData) -> String {
        let imageFetchURL = (sessionData.serverURL + RESTServer.REST_USER_GET_DOCUMENT_PAGE_THUMBNAIL)
        let imageFetchURLReplaced = imageFetchURL.replacingOccurrences(of: "{doc_id}", with: String(pageId))
        return imageFetchURLReplaced
    }
